...
- **验证码提取** — 自动从收到的邮件中提取 OTP/验证码，支持 **6 种语言**（中文、英文、日文、俄文、法文、德文）
...
等待验证邮件并自动提取验证码。支持 **6 种语言**的验证邮件：中文（简体/繁体）、英文、日文、俄文、法文、德文。

采用多策略提取算法：
1. **上下文匹配** — 查找各语言关键词（如 `验证码`、`code`、`認証`、`код`、`vérification`、`Bestätigung`）附近的验证码
2. **独立数字验证码** — 回退到纯数字验证码（优先 6 位），自动过滤年份等误匹配
3. **字母数字验证码** — 最后手段回退到大写字母数字组合，过滤纯字母单词

- **address**：临时邮箱地址。
- **subjectContains**（可选）：过滤器（如 `"verification"`、`"验证"`、`"code"`、`"Код"`）。
